...
import { sendMessage, deleteSample } from '../../../store/actions'
...
`
	...
	if (!rackData.isOpen) {
		columns.splice(
			...
			0,
			{
				title: 'Instrument',
				align: 'center',
				render: record => (record.instrument ? record.instrument : 'N/A')
			},
			{
				title: 'Holder',
				align: 'center',
			...
			},
			{
				title: 'Status',
				align: 'center',
				render: record => (record.status ? record.status : 'N/A')
			}
		)
	}
	...
	const rowSelection = !rackData.isOpen && {
		...
		// 	setSelectedSlots(selectedRowKeys)
		// }
	}
